...
namespace THA_W7_Benito
{
    public partial class Form1 : Form
    {
        ...
        List<int> selectedMovieSeatNumber = new List<int>();
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            foreach (string line in lines)
            {
                ...
                movieimage.Click += (sendter, EventArgs) => { image_click(movies.IndexOf(movieclass), movieclass); };
        ...
        void image_click(int index, MovieClass movieClass)
        {
            panel2.Visible = true;
            moviePointer = index;
            ...
            selectedMovieSeatNumber.Clear();
            labelSelectedSeat.Text = "";
            ...
            foreach(string time in movieClass.TimeArray)
            {
                ...
                pilihan.Text = time;
                pilihan.Click += (sendter, EventArgs) => { generateCheckBoxKursi(movieClass, time); };
                if (counter == 0)
                {
            ...
            for(int i=1; i<101; i++)
            {
                ...
                tempCheckbox.CheckedChanged += (sendter, EventArgs) => { handleSeatChanges(tempCheckbox); };
        ...
        void handleSeatChanges(CheckBox checkbox)
        {
            if(checkbox.Checked)
            {
                selectedMovieSeatNumber.Add(int.Parse(checkbox.Text));
            }
            else
            {
                selectedMovieSeatNumber.Remove(int.Parse(checkbox.Text));
            }

            labelSelectedSeat.Text = "";
            for(int i=0; i < selectedMovieSeatNumber.Count; i++)
            {
                if(i == 0)
                    labelSelectedSeat.Text += "Selected: " + selectedMovieSeatNumber[i];
                else
                    labelSelectedSeat.Text += ", " + selectedMovieSeatNumber[i];
            }
